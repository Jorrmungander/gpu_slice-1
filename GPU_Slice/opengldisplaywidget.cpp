...
OpenGLDisplayWidget::OpenGLDisplayWidget(QWidget *parent): QOpenGLWidget(parent), distanceToCamera(-8.0)
{
...
OpenGLDisplayWidget::~OpenGLDisplayWidget()
{
    ...
    delete streamLinesRenderer;
    delete streamLinesMapper;
}
...
void OpenGLDisplayWidget::paintGL()
{
    ...
    horizontalContourLinesRenderer->drawContourLines(mvpMatrix);
    streamLinesRenderer->drawStreamLines(mvpMatrix, mvpMatrix);
}
...
void OpenGLDisplayWidget::keyPressEvent(QKeyEvent *e)
{
    if (e->key() == Qt::Key_Up)
    {
        if(horizontalSliceRenderer->z < 15){
            horizontalSliceRenderer->moveSlice(1);
            horizontalContourLinesRenderer->moveSlice(1);
        }
    }
    else if (e->key() == Qt::Key_Down)
    {
        if(horizontalSliceRenderer->z > 0){
            horizontalSliceRenderer->moveSlice(-1);
            horizontalContourLinesRenderer->moveSlice(-1);
        }
    }
    ...
    {
        if(streamLinesRenderer->time < 100){
            //initVisualizationPipeline(streamLinesRenderer->time+1);
        }
    }
    else if (e->key() == Qt::Key_P)
    {
        if(streamLinesRenderer->time > 0){
            //initVisualizationPipeline(streamLinesRenderer->time-1);
        }
    }
    else return;
...
void OpenGLDisplayWidget::initVisualizationPipeline()
{
    ...
    source = new FlowDataSource();

    source->createData(0);
    source->printValuesOfHorizontalSlice(9);

    // Initialize mapper modules.
    std::cout << "Initialize mapper.\n";
    horizontalSliceToImageMapper = new HorizontalSliceToImageMapper();
    horizontalSliceToImageMapper->setDataSource(source);

    horizontalSliceToContourLineMapper = new HorizontalSliceToContourLineMapper(source);

    // Initialize rendering modules.
    std::cout << "Inizialize renderingmodels.\n";
    ...
    horizontalContourLinesRenderer = new HorizontalContourLinesRenderer(horizontalSliceToContourLineMapper);
}
